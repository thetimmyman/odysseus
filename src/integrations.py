...
# ---------------------------------------------------------------------------
# Outbound fetch pinning (ports upstream #5727).
#
# execute_api_call runs check_outbound_url on the target, but that guard only
# resolves the host to answer (ok, reason) and hands back no address. The
# request right after it opened a plain httpx.AsyncClient, which resolves the
# host again at connect time. A base_url host on a low TTL can pass the guard
# as a public IP and then flip to 169.254.169.254 for the connect, so the call
# lands on cloud metadata with the integration's stored auth headers attached.
#
# Resolve once, remember the IPs the guard actually validated, and pin the
# client's socket to that set through a small AnyIO-backed transport. SNI and
# the Host header still come from the URL, so TLS and vhost routing are
# unchanged; connect-time fallback stays inside the approved address set over
# one shared deadline.
# ---------------------------------------------------------------------------
...
async def execute_api_call(
    ...
    def _recording_resolver(host: str) -> List[str]:
        _ips = _default_resolver(host)
        _resolved_ips[:] = _ips
        return _ips

    _ok, _reason = check_outbound_url(
        url, block_private=_block_private, resolver=_recording_resolver
    )
    if not _ok:
        return {"error": f"URL rejected: {_reason}", "exit_code": 1}
    _pinned_ips = _validated_ips(_resolved_ips)
    if not _pinned_ips:
    ...
    try:
        async with httpx.AsyncClient(
            timeout=30.0, transport=_PinnedAsyncTransport(_pinned_ips)
        ) as client:
